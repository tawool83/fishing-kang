 ...
 * 원본은 `assets/bell-dinging-jam-fx-1-1-00-04.mp3`, 배포본은 `public/bell.mp3`.
 *
 * 파일은 약 204KB다. 그래서 **모듈을 불러올 때 받지 않는다** — 첫 사용자 제스처에
 * `unlock()`이 불릴 때 비로소 `Audio`를 만든다. 조과 화면이 뜨는 길목에 204KB를
 * 끼워 넣으면 약한 전파에서 첫 화면이 그만큼 늦어지는데, 소리는 늦게 와도 된다.
